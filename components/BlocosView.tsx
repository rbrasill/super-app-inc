...
function InfoTile({ value, label, sub, subColor }: { value: string; label: string; sub?: string; subColor?: string }) {
  return (
    <div className="bg-panel border border-line rounded-2xl shadow-soft px-5 py-4 flex-1 min-w-[150px]">
      <div className="font-head text-[26px] font-extrabold tracking-[-0.03em] leading-none">{value}</div>
      <div className="text-[11px] font-bold uppercase tracking-[0.4px] text-inkSoft mt-[7px]">{label}</div>
      {sub && (
        <div className="text-[11px] font-bold mt-[3px]" style={{ color: subColor }}>
          {sub}
        </div>
      )}
    </div>
...
function Timeline({ rows }: { rows: BlockRow[] }) {
  ...
  return (
    <div className="bg-panel border border-line rounded-2xl shadow-soft p-5">
      <div className="flex items-center gap-[9px] text-[12.5px] font-extrabold uppercase tracking-[0.5px] mb-4">
      ...
      {/* Régua de semanas */}
      <div className="flex justify-between mt-2 text-[10px] font-bold text-inkFaint">
        <span>Semana 1</span>
...
function BlockCard({ row, tasks }: { row: BlockRow; tasks: Task[] }) {
  ...
  return (
    <div
      className="bg-panel border border-line rounded-2xl shadow-soft overflow-hidden flex flex-col"
      style={{ borderTop: `4px solid ${row.color}` }}
    >
      {/* Cabeçalho */}
      <div className="px-5 pt-4 pb-3 border-b border-line2">
        <div className="flex items-center gap-2 mb-2">
          <span
            className="text-[10px] font-extrabold uppercase tracking-[0.4px] px-2 py-[3px] rounded-[20px] text-white inline-flex items-center gap-[3px]"
          ...
          <div className="ml-auto flex items-center gap-1">
            <button
              ...
              disabled={isFirst}
              className="w-6 h-6 rounded-md text-inkFaint hover:bg-chip hover:text-ink disabled:opacity-30 disabled:hover:bg-transparent transition-colors text-[13px] font-bold"
        ...
        <div className="font-head text-[16px] font-extrabold tracking-[-0.02em]">{row.name}</div>
      ...
      <div className="px-5 py-3 border-b border-line2">
        <div className="flex items-center gap-[10px]">
          ...
          <span className="text-[12px] font-extrabold">{row.txt}</span>
        ...
        <div className="text-[10px] font-extrabold text-inkSoft text-right mt-1">{row.pctLabel} entregue</div>
      </div>

      {/* Distribuição por área */}
      {row.areaSegs.length > 0 && (
        <div className="px-5 py-3 border-b border-line2">
          <div className="text-[10px] font-extrabold uppercase tracking-[0.5px] text-inkFaint mb-2">Por área</div>
      ...
      <div className="px-5 py-3 flex-1">
        {items.length === 0 ? (
          <div className="text-[11.5px] text-inkFaint italic font-medium py-2">
          ...
          <div className="flex flex-col gap-[7px]">
            {items.map((tk) => {
              ...
              return (
                <div key={tk.id} className="flex items-center gap-2 text-[11.5px]">
                  <span className="w-[8px] h-[8px] rounded-full flex-shrink-0" style={{ background: st?.color }} />
                  <span className="font-semibold truncate flex-1">{tk.desc}</span>
                  <span
...
export default function BlocosView() {
  ...
  return (
    <div className="pt-[14px] flex flex-col gap-4">
      {/* Intro + ação */}
      <div className="bg-kpiDark text-white rounded-[18px] px-7 py-[22px] relative overflow-hidden flex items-center gap-4 flex-wrap">
        <div className="pointer-events-none absolute -right-10 -top-10 w-[170px] h-[170px] bg-accent opacity-20 rounded-full" />
        <div className="min-w-0 flex-1">
          <div className="text-[11px] font-extrabold tracking-[1px] uppercase text-accent">Estratégia de blocos</div>
          <div className="font-head text-[22px] font-extrabold mt-[6px] mb-[4px]">O boi é o app · cada bife é um bloco</div>
          <div className="text-[12.5px] text-white/70 font-medium max-w-[620px] leading-[1.5]">
      ...
      {blocks.length === 0 ? (
        <div className="bg-panel border border-dashed border-line rounded-2xl p-10 text-center">
          ...
          <div className="font-head text-[16px] font-extrabold mb-1">Nenhum bloco ainda</div>
