...
const EMPTY: PersonInput = { name: "", role: "", resp: "" };

function toInput(p: Person): PersonInput {
  const { name, role, resp } = p;
  return { name, role, resp };
}
...
export default function PersonModal() {
  const { personModal, people, tasks, addPerson, updatePerson, deletePerson, closePersonModal } = useStore();
  ...
  const submit = () => {
    ...
    const clean: PersonInput = { name: form.name.trim(), role: form.role.trim(), resp: form.resp.trim() };
