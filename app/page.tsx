...
function DbErrorToast() {
  const { dbError, clearDbError } = useStore();
  if (!dbError) return null;
  const title =
    dbError.kind === "load"
      ? "Sem conexão com o banco — mostrando dados de demonstração"
  ...
  return (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 max-w-[520px] flex items-start gap-3 rounded-[14px] border border-danger/30 bg-white px-4 py-3 shadow-xl">
      ...
      <div className="min-w-0">
        <div className="text-[12.5px] font-bold text-inkDark">{title}</div>
        <div className="text-[11.5px] text-inkSoft mt-[2px] break-words">{dbError.message}</div>
      </div>
      <button
